...
import sys
from PyQt5.QtWidgets import QApplication, QMainWindow, QVBoxLayout, QSlider, QDialog, QLabel, QHBoxLayout, QPushButton
from PyQt5.QtCore import Qt

#epsilon改变滑块
class EpsilonDialog(QDialog):
    epsilon_changed = QtCore.pyqtSignal(float)
    def __init__(self, current_epsilon, parent=None):
        ...
        self.slider.setValue(int(self.epsilon))  # 设置滑块初始值
        self.slider.valueChanged.connect(self.update_epsilon)

        self.epsilon_label = QLabel(f"Epsilon: {self.epsilon}", self)
    ...
    def update_epsilon(self, value):
        ...
        self.epsilon_changed.emit(self.epsilon)
...
class HoverWindow(QWidget):
    def __init__(self, initial_image):
        ...
        self.title_label.setStyleSheet("font-weight: bold;")  # 设置标题文字加粗

        layout = QVBoxLayout(self)
        layout.addWidget(self.title_label)
        layout.addWidget(self.image_label)

        self.update_content(initial_image, "")

    def update_content(self, pixmap, title, extend = 200):
            if pixmap is None:
                self.close()
                return
            ...
            # 判断最长边是否小于 extend
            if max(image_size.width(), image_size.height()) > extend:
                # 计算缩放比例
                scale_factor = extend / max(image_size.width(), image_size.height())
                # 缩放图片
                pixmap = pixmap.scaled(image_size * scale_factor)
    
            self.image_label.setPixmap(pixmap)
            self.title_label.setText(title)

            # 计算窗口的最小宽度，确保比图片和标题都大一些
            min_width = max(pixmap.width(), self.title_label.sizeHint().width()) + 10  # 添加额外的空间

            # 调整窗口大小以适应图片和标题
            self.setFixedSize(min_width, pixmap.height() + self.title_label.sizeHint().height() + 10)  # 设置额外的高度
            self.show()

    # def update_content(self, pixmap, title):
    ...
    #     self.title_label.setText(title)
        ...
        # # 计算窗口的最小宽度，确保比图片和标题都大一些
        # min_width = max(image_size.width(), title_size.width()) + 10  # 添加额外的空间

        # # 调整窗口大小以适应图片和标题
        # self.setFixedSize(min_width, image_size.height() + title_size.height() + 10)  # 设置额外的高度
        # self.show()
...
class Canvas(
    ...
    shape_change_track_id = QtCore.pyqtSignal(int, str, int)
    paste_track_id_end = QtCore.pyqtSignal(str)
    shape_moved_singnal = QtCore.pyqtSignal(list)
    ...
    def __init__(self, *args, **kwargs):
        ...
        # track
        self._continue_track_crop_image = None
        self.hover_window = None  # 初始化悬浮窗口
        self.continue_track_id = None

        self.paste_track_shapes_list = []
        self.paste_track_mode = 0
        self.current_pos = None #记录当前鼠标pos位置
        self.record_pos = None #记录paste track初始鼠标位置
        self.record_shapes_index = 0 # 记录shapes列表该index后都是暂存的
        ...
        self.first_region_point = None

    def is_rect_outside(self, rect, reference_rect):
        # rect: (left, top, right, bottom)
        # reference_rect: (left, top, right, bottom)

        # 判断矩形是否在reference_rect的左侧、右侧、上方或下方
        outside_left = rect[2] < reference_rect[0]
        outside_right = rect[0] > reference_rect[2]
        outside_top = rect[3] < reference_rect[1]
        outside_bottom = rect[1] > reference_rect[3]

        # 如果矩形完全在reference_rect的左侧、右侧、上方或下方，则认为矩形完全不相交
        return outside_left or outside_right or outside_top or outside_bottom

    def match_track_shapes(self, iou_thresh = 0.5, second = False):
        # 将确定好的tracks匹配修改当前帧中shapes的label group_id
        shapes_to_remove_index = []
        # 超出边界的直接结束追踪
        have_pt_shape = False
        for i, shape in enumerate(self.shapes):
            size = self.pixmap.size()
            if shape.description == "pt":
            #限制
                have_pt_shape = True
                if second:  
                    shapes_to_remove_index.append(i)
                else:
                    if self.is_rect_outside([shape.points[0].x(),shape.points[0].y(),shape.points[2].x(),shape.points[2].y()],
                                    [0,0,size.width(),size.height()]):
                        self.paste_track_id_end.emit(f"{shape.label}_{shape.group_id}")
                        shapes_to_remove_index.append(i)
        if not have_pt_shape:
            return

        ious_all = [] #[[index1, 对应的匹配结果], [index2, 对应的匹配结果]] index是待匹配框的索引
        for i, shape in enumerate(self.shapes):
            if shape.description != "pt" and (not second or shape.group_id is None):#对待标注框
                ious = []
                for j, shape_ in enumerate(self.shapes):
                    if shape_.description == "pt" and (second or j not in shapes_to_remove_index):
                        iou = calculate_iou(QtCore.QRectF(shape.points[0],shape.points[2]), QtCore.QRectF(shape_.points[0],shape_.points[2]))
                        ious.append([j,iou])
                #按照iou从大到小排序
                sorted_ious = sorted(ious, key=lambda x: x[1], reverse=True)
                filtered_ious = [elem for elem in sorted_ious if elem[1] > 0]
                if len(filtered_ious) == 0:#没有iou大于0的pt对象
                    continue
                ious_all.append([i, filtered_ious])
        
        save_shape_index = []
        match_shape_index = []
        for index, filtered_ious in ious_all:
            if filtered_ious[0][1]>iou_thresh:
                shape_ = self.shapes[filtered_ious[0][0]]
                print(f"iou: {filtered_ious[0][1]}, {shape_.label}, {shape_.group_id}")
                shapes_to_remove_index.append(filtered_ious[0][0])#匹配
                match_shape_index.append(filtered_ious[0][0])
                self.shape_change_track_id.emit(index, shape_.label, shape_.group_id)
            else:
                #将iou_thresh>iou>0的这部分中保留交集最大的部分,其他都remove
                save_shape_index.append(filtered_ious[0][0])
                for j, iou in filtered_ious[1:]:
                    shapes_to_remove_index.append(j)
        unique_shapes_to_remove_index = list(set(shapes_to_remove_index))
        unique_save_shape_index = list(set(save_shape_index))
        # match成功的一定不会保存
        unique_save_shape_index = [i for i in unique_save_shape_index if i not in match_shape_index]
        # 未match 且在某个待标注的框中iou排第一的pt框需要保存下来
        unique_shapes_to_remove_index = [i for i in unique_shapes_to_remove_index if i not in unique_save_shape_index]
        
        remove_shapes = [shape for i, shape in enumerate(self.shapes) if i in unique_shapes_to_remove_index]
        
        # 从 self.shapes 中移除匹配的框的元素
    ...
    # 记录当前鼠标位置
    def paste_tracks_recordmousePos(self):
        self.record_pos = QPointF(self.current_pos)
        self.record_shapes_index = len(self.shapes)

    def update_paste_tracks(self, pos):
        delate_pos = self.record_pos - pos
        self.record_pos = pos
        for i, shape in enumerate(self.shapes):
            if shape.description == "pt":
                self.shapes[i].points = [point - delate_pos for point in self.shapes[i].points]
    

    @property
    def continue_track_crop_image(self):
        return self._continue_track_crop_image

    @continue_track_crop_image.setter
    def continue_track_crop_image(self, pixmap):
        self.init_hover()
        
        if not self._continue_track_crop_image or not self._continue_track_crop_image.isNull():
            # 在这里设置悬浮窗口显示的图像
            self.update_hover_window_content(pixmap)
            
        self._continue_track_crop_image = pixmap
    
    def update_hover_window_content(self, pixmap):
        if self.continue_track_crop_image is not None:
            if self.hover_window is not None:
                self.hover_window.update_content(pixmap, self.continue_track_id)

    def show_hover_window(self, event):
        # 更新悬浮窗口位置
        self.hover_window.move(event.globalPos() + QtCore.QPoint(10, 10))

        # 检查是否要显示悬浮窗口
        if self.hover_window.isVisible():
            self.hover_window.show()

    def init_hover(self):
        if self.hover_window is None:
            self.hover_window = HoverWindow(self.continue_track_crop_image)
            self.hover_window.setFixedSize(200, 200)  # 设置悬浮窗口的大小
            self.hover_window.move(QtCore.QPoint(10, 10))  # 设置初始位置为鼠标当前位置
            self.hover_window.setWindowFlag(Qt.WindowStaysOnTopHint)  # 将窗口置于最顶层
            if self.hover_window.isVisible():
                self.hover_window.show()
            return 
    ...
    def mouseMoveEvent(self, ev):  # noqa: C901
        ...
        if self.paste_track_mode == 1: #更新整体浮动位置，对每个矩形的位置减去鼠标相对位移
            self.update_paste_tracks(pos)
            return
        ...
        if QtCore.Qt.LeftButton & ev.buttons():
            if self.first_region_point:
                # 绘制矩形，使用 self.first_region_point 作为一个固定点
                x1, y1 = self.first_region_point.x(), self.first_region_point.y()
                x2, y2 = pos.x(), pos.y()
                min_x = min(x1, x2)
                max_x = max(x1, x2)
                min_y = min(y1, y2)
        ...
        for shape in reversed([s for s in self.shapes if self.is_visible(s)]):
            ...
            if shape.contains_point(pos):
                ...
                self.h_edge = None
                self.setToolTip(
                    self.tr("Click & drag to move shape '%s'") % f"{shape.label}_{shape.group_id}"
                )
                self.setStatusTip(self.toolTip())
    ...
    def have_pt_shape(self,):
        have_pt_shape_ = False
        for i, shape in enumerate(self.shapes):
            if shape.description == "pt":
                have_pt_shape_ = True
        return have_pt_shape_
            ...
            if self.drawing():
                if self.current:
                    ...
                    if self.create_mode == "point":
                        ...
                        self.update()
            elif self.paste_track_mode == 1: #整体浮动状态
                self.paste_track_mode = 2 #进入单独调整阶段
                QtWidgets.QMessageBox.information(
                        None,
                        "右键提示",
                        f"调整pt框, pt与目标框重合度超过60%，再点击一下右键进行追踪",
                ...
                if group_mode:
                    self.first_region_point = pos
        elif ev.button() == QtCore.Qt.RightButton and self.editing():
            group_mode = int(ev.modifiers()) == QtCore.Qt.ControlModifier

            if self.paste_track_mode == 3: #二次调整确认
                self.match_track_shapes(iou_thresh=0.01, second = True)
                QtWidgets.QMessageBox.information(
                        None,
                        "结束",
                        f"追踪结束",
                ...
                self.paste_track_mode = 4
                
            if self.paste_track_mode == 2: #单独调整阶段确认
                self.match_track_shapes(iou_thresh=0.5, second = False)
                if self.have_pt_shape():
                    self.paste_track_mode = 3
                    QtWidgets.QMessageBox.information(
                        None,
                        "右键提示",
                        f"继续调整pt, 此时只需要和白色待标记框接触即可，再点击一下右键进行二次追踪",
                        QtWidgets.QMessageBox.Ok,
                    )
                else:
                    QtWidgets.QMessageBox.information(
    ...
    def mouseReleaseEvent(self, ev):
        ...
        if ev.button() == QtCore.Qt.RightButton:
            ...
            if self.editing():
                ...
                group_mode = int(ev.modifiers()) == QtCore.Qt.ControlModifier
                if group_mode and self.first_region_point:
                    pos = self.transform_pos(ev.localPos())
                    region = (self.first_region_point, pos)
                    self.select_shape_rect(region)
                    self.first_region_point = None
    ...
    def is_rect_contains_points(self, region, rect):
        assert len(region) == 2
        x1, y1 = region[0].x(), region[0].y()
        x2, y2 = region[1].x(), region[1].y()
        min_x = min(x1, x2)
        max_x = max(x1, x2)
        min_y = min(y1, y2)
        max_y = max(y1, y2)
        rectangle = QtCore.QRectF(min_x, min_y, max_x - min_x, max_y - min_y)

        return rectangle.contains(rect)

    def select_shape_rect(self, region):
        for shape in reversed(self.shapes):
            rec_shape = shape.bounding_rect()
            if self.is_visible(shape) and self.is_rect_contains_points(region, rec_shape):
                self.set_hiding()
    ...
    def bounded_move_shapes(self, shapes, pos):
        ...
        if dp:
            for shape in shapes:
                shape.move_by(dp)
            self.shape_moved_singnal.emit(shapes)
            self.prev_point = pos
            return True
        return False
