...
class LabelListWidget(QtWidgets.QListView):
    ...
    def add_iem(self, item, group_id):# 追踪模式下self lable的顺序最上面是group_Id为None
        if not isinstance(item, LabelListWidgetItem):
            raise TypeError("item must be LabelListWidgetItem")
        self.model().setItem(self.model().rowCount(), 0, item)
        item.setSizeHint(self.itemDelegate().sizeHint(None, None))

    def move_top(self, item): #将当前item移动到顶端
        current_row = self.model().indexFromItem(item)
        self.model().takeItem(current_row.row(), 1)
        # self.model().removeRows(current_row.row(), 1)
        self.model().setItem(self.model().rowCount(), 0, item)
    ...
    def hide_item(self, item, hidden=True):
        index = self.model().indexFromItem(item)
        if hidden:
