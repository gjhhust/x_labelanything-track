...
class LabelConverter:
    ...
    def coco_to_custom_frame(self, input_file, image_path, cur_image_file):
        with open(input_file, "r", encoding="utf-8") as f:
            data = json.load(f)
        ...
        for dic_info in total_info.values():
            ...
            if cur_image_file !=  osp.basename(dic_info["imagePath"]):
                continue
    ...
    def find_similar_string(self, query, string_list, threshold=80):
        """
        在字符串列表中找到与查询字符串相似度高于阈值的字符串，并返回最可能的匹配结果。
        """
        similar_strings = [(s, fuzz.ratio(query, s)) for s in string_list]
        similar_strings = sorted(similar_strings, key=lambda x: x[1], reverse=True)
        
        # 如果最高相似度大于阈值，返回最可能的匹配结果
        if similar_strings and similar_strings[0][1] >= threshold:
            return similar_strings[0][0]
        else:
            return None


    def custom_to_coco(self, input_path, output_path, require_box=None):
        ...
        for file_name in label_file_list:
            ...
            for shape in data["shapes"]:
                ...
                if label not in self.classes:
                    path = data["imagePath"]
                    result = self.find_similar_string(label, self.classes)
                    if require_box is not None:
                        if require_box(label, result):
                            label = result
                    else:
                        raise f"{label} in {path}\n{label} not in {self.classes}"
